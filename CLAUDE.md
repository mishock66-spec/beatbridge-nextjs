...
1. ALWAYS save artist photo to /public/artists/[slug].jpg — no placeholder allowed. NEVER use unavatar.io — it returns generic 1.5KB placeholders at build time. The real photo must be manually downloaded (e.g. from the artist's Instagram in a browser → right-click → Save Image) and saved to /public/artists/[slug].jpg. Verify the file is >10KB before committing. Reference path in code: `/artists/[slug].jpg`. All photo references across artists/page, page.tsx, dashboard/page, and dedicated artist pages must point to this path.
