...
When asked to add a new artist to BeatBridge, follow the ARTIST_ONBOARDING.md pipeline exactly, step by step.
...
- Statut de contact → "À contacter" (fixed for all rows)
- Suivi par → Artist name (fixed for all rows)
- template → empty
- follow_up → empty
...
- Templates must follow the format: "Hey [name], I'm [BEATMAKER_NAME], a beatmaker. ..."
...
- Create /artist/[slug] page with follower-range sub-pages: 0-500, 500-5K, 5K-10K, 10K-20K, 20K-30K, 30K-40K, 40K-50K
- Max 50 contacts per page with pagination
- Create Top Contacts page (max 50 curated contacts)
- Add artist photo to /public/images/[slug].jpg — NEVER /public/artists/ (Next.js routing conflict)
- Find artist photos on studiotalksevents.com first; file must be >10KB
...
- Use window.location.href on mobile, window.open on desktop

## CRITICAL RULES (never override)
- NEVER put artist photos in /public/artists/ — Next.js routing conflict
- ALWAYS revalidate = 0 on all pages
...
- Instagram usernames: clickable links to profile wall (not ig.me)
- Every new page must be accessible from navbar or footer`;
