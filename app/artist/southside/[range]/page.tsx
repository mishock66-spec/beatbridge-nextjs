...
const DEFAULT_TEMPLATE =
  "Yo, caught you in Southside's circle — got some hard trap beats I've been sitting on, think it could fit your lane?";
...
export default async function SouthsideRangePage({
  params,
  searchParams,
}: {
  params: { range: string };
  ...
  try {
    const raw = await fetchAirtableRecords("Southside", {
      min: config.min,
      max: config.max,
    });
  ...
  const pageUrl = (p: number) =>
    p === 1 ? `/artist/southside/${range}` : `/artist/southside/${range}?page=${p}`;

  return (
    <AuthGateClient redirectUrl={`/artist/southside/${range}`}>
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-2 text-sm mb-10 flex-wrap">
          ...
          <Link href="/artist/southside" className="text-gray-500 hover:text-orange-500 transition-colors">
            Southside
          </Link>
          <span className="text-gray-600">›</span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end gap-6 mb-8">
          <div className="w-20 h-20 rounded-xl bg-[#111111] border border-[#1f1f1f] overflow-hidden flex-shrink-0">
            ...
            <img src="/images/southside.jpg" alt="Southside" className="w-full h-full object-cover" />
          </div>
          <div className="flex-1">
            <p className="text-[#606060] text-xs uppercase tracking-[0.1em] mb-1">
              Southside · Full Network
            </p>
            ...
            <p className="text-[#a0a0a0] text-sm">
              Contacts following Southside with {config.label} followers
            </p>
          </div>
          ...
          <div className="bg-orange-500/[0.10] border border-orange-500/30 rounded-xl px-4 py-3 mb-8 flex items-start gap-3">
            ...
            <p className="text-sm text-[#a0a0a0] leading-relaxed">
              <span className="text-orange-400 font-semibold">
                Under 500 followers — these are the most reachable people in Southside&apos;s network.
              </span>{" "}
        ...
        <div className="mt-8 flex items-center justify-between gap-4 border-t border-[#1f1f1f] pt-8">
          {config.prev ? (
            <Link href={`/artist/southside/${config.prev}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-orange-500 transition-colors">
              ← {RANGE_CONFIG[config.prev].label}
            </Link>
          ) : <div />}
          {config.next ? (
            <Link href={`/artist/southside/${config.next}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-orange-500 transition-colors">
        ...
        <div className="mt-8 flex justify-center">
          <Link href="/artist/southside" className="text-sm text-gray-500 hover:text-orange-500 transition-colors">
            ← Back to Southside
          </Link>
        </div>
