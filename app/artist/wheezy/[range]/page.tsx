...
const DEFAULT_TEMPLATE =
  "Yo, I noticed Wheezy follows you — that says a lot about who you are in the space. I'm a beatmaker and I've been working on some heat lately. Would mean the world if you could pass this along to him or anyone in your circle who might vibe with it: [LINK]";
...
export default async function WheezyRangePage({
  params,
}: {
  params: { range: string };
}) {
  ...
  try {
    const raw = await fetchAirtableRecords("Wheezy", {
      min: config.min,
      max: config.max,
    });
    records = raw.map((r) => ({
  ...
  return (
    <AuthGateClient redirectUrl={`/artist/wheezy/${range}`}>
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="mt-12 flex items-center justify-between gap-4 border-t border-[#1f1f1f] pt-8">
          {config.prev ? (
            <Link
              href={`/artist/wheezy/${config.prev}`}
          ...
          {config.next ? (
            <Link
              href={`/artist/wheezy/${config.next}`}
              className="flex items-center gap-2 text-sm text-gray-500 hover:text-orange-500 transition-colors"
            >
