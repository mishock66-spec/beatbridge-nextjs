...
const TOP_CAP = 50;
const FOLLOWER_CAP = 50_000; // Harry Fraud ~200K — cap at 50K
...
function selectTopContacts(all: AirtableRecord[]): AirtableRecord[] {
  ...
  // Tier 2: Artist/Rapper — with bio + template, fill remaining spots
...
export default async function HarryFraudTopContactsPage() {
  let records: AirtableRecord[] = [];
  let error: string | null = null;

  try {
    const all = await fetchAirtableRecords("Harry Fraud");
  ...
  return (
    <AuthGateClient redirectUrl="/artist/harry-fraud/top">
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-2 text-sm mb-10 flex-wrap">
          ...
          <Link href="/artist/harry-fraud" className="text-gray-500 hover:text-orange-500 transition-colors">
            Harry Fraud
          </Link>
          <span className="text-gray-600">›</span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end gap-6 mb-8">
          <div className="w-20 h-20 rounded-xl bg-[#111111] border border-[#1f1f1f] overflow-hidden flex-shrink-0">
            ...
            <img src="/images/harryfraud.jpg" alt="Harry Fraud" className="w-full h-full object-cover" />
          </div>
          <div className="flex-1">
            <p className="text-[#606060] text-xs uppercase tracking-[0.1em] mb-1">
              Harry Fraud · Curated List
            </p>
            ...
            <p className="text-[#a0a0a0] text-sm">
              The {records.length} highest-value contacts in Harry Fraud&apos;s network — under 50K followers, producers, engineers, managers first
            </p>
          </div>
        ...
        <ArtistNetworkClient
          ...
          artistSlug="harry-fraud"
        />

        <div className="mt-12 flex justify-center border-t border-[#1f1f1f] pt-8">
          <Link href="/artist/harry-fraud" className="text-sm text-gray-500 hover:text-orange-500 transition-colors">
            ← Back to Harry Fraud
          </Link>
        </div>
