...
function DemoConnectionCard({ record }: { record: AirtableRecord }) {
  ...
  function handleGated(e: React.MouseEvent) {
    e.preventDefault();
    setGated(true);
    setTimeout(() => setGated(false), 4000);
  }
    ...
    <div className="bg-white/[0.025] backdrop-blur-md border border-white/[0.08] rounded-2xl p-6 flex flex-col gap-4 hover:border-white/[0.15] hover:-translate-y-0.5 transition-all duration-200 hover:shadow-xl hover:shadow-black/30 relative">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 pr-24">
          ...
          <p className="text-orange-400 text-sm">@{record.username.replace("@", "")}</p>
          {record.followers > 0 && (
      ...
      {/* Step 1 — Ice Breaker (fully visible in demo) */}
      {record.template && (
        <div className="bg-white/[0.02] rounded-xl p-3.5 border border-white/[0.06]">
          ...
          {gated && (
            <div className="mt-3 flex items-center justify-center gap-2 text-sm">
              ...
              <Link
                href="/sign-up"
                className="text-orange-400 font-semibold hover:text-orange-300 transition-colors hover:underline"
              >
                Sign in to use →
              </Link>
            </div>
      ...
      {/* Status pill — locked in demo */}
      <div className="flex items-center gap-2">
        <div
          className="text-xs font-medium px-2.5 py-1 rounded-full flex items-center gap-1.5 opacity-40 cursor-not-allowed select-none"
          style={{
        ...
        >
          <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 bg-[#374151]" />
          To contact
          <span className="text-[10px] ml-1">🔒</span>
        </div>
        <span className="text-[11px] text-[#404040]">Status tracking requires an account</span>
      </div>
    </div>
  );
}
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-[#a0a0a0] text-sm mt-1">
          {records.length} contacts in the {rangeLabel} range — fully real data, no sign-in required.
        </p>
      </div>
      ...
      <div className="bg-orange-500/[0.06] border border-orange-500/15 rounded-xl px-4 py-3 mb-8 flex items-start gap-3">
        ...
        <p className="text-sm text-[#a0a0a0] leading-relaxed">
          ...
          Start with 500–5K for best results. DM templates are fully visible — sign up to copy and send them.
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((record, index) => (
            ...
            >
              <DemoConnectionCard record={record} />
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-16 border-t border-white/[0.05] pt-12 text-center">
        ...
        <p className="text-[#a0a0a0] text-sm mb-8 max-w-md mx-auto">
          Create a free account to copy DM templates, track your outreach, and access every artist&apos;s full network.
        </p>
