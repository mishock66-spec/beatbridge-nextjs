 ...
 *
 * Run: node scripts/process-juke-wong.mjs
 */
...
function classifyFromNotes(notes) {
  if (!notes || !notes.trim()) return null; // No bio → keep as Autre
  ...
  return null; // Cannot determine → keep as Autre
}
...
function generateTemplate(record) {
  ...
  // Build a reference line based on what the bio says
  let reference;
  if (notes.includes("producer") || notes.includes("beatmaker") || notes.includes("making beats")) {
    ...
    reference = "caught your page through Juke Wong's network";
  } else {
    reference = "caught you in Juke Wong's circle";
  }

  // Tailor the pitch based on who they are
  let pitch;
  if (notes.includes("rapper") || notes.includes("artist") || notes.includes("vocalist") || notes.includes("singer")) {
    pitch = "got some melodic trap beats that might fit your sound";
  } else if (notes.includes("producer") || notes.includes("beatmaker")) {
    pitch = "got some melodic trap joints we could build on";
  } else if (notes.includes("manager") || notes.includes("label") || notes.includes("a&r")) {
    pitch = "got some melodic trap beats worth passing along";
  } else {
    pitch = "got some melodic trap beats I've been sitting on";
  }

  return `Yo ${name}, ${reference} — ${pitch}, think it could fit your lane?`;
}

// ─── Airtable helpers ─────────────────────────────────────────────────────────

async function fetchAllJukeWong() {
  const records = [];
  let offset = null;
  ...
  do {
    const parts = [
      "pageSize=100",
      "filterByFormula=" + encodeURIComponent('{Suivi par}="Juke Wong"'),
...
async function main() {
  console.log("Fetching all Juke Wong records from Airtable…");
  const records = await fetchAllJukeWong();
