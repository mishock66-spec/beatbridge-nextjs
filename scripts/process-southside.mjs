 ...
 *
 * Run: node scripts/process-southside.mjs
 */
...
function generateTemplate(record) {
  ...
  if (notes.includes("producer") || notes.includes("beatmaker") || notes.includes("making beats")) {
    ...
    reference = "caught your page through Southside's network";
  } else {
    reference = "caught you in Southside's circle";
  }
...
async function fetchAllSouthside() {
  const records = [];
  let offset = null;
  ...
  do {
    const parts = [
      "pageSize=100",
      "filterByFormula=" + encodeURIComponent('{Suivi par}="Southside"'),
...
async function main() {
  console.log("Fetching all Southside records from Airtable…");
  const records = await fetchAllSouthside();
